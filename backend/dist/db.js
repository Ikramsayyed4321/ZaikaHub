...
async function seedData(db) {
    ...
    if (isDevelopment) {
       ...
       VALUES (?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE password_hash = VALUES(password_hash), role = VALUES(role), is_active = VALUES(is_active), restaurant_id = VALUES(restaurant_id)`, [1, 'Zaika Hub Admin', seedAdminEmail, hash, 'admin', true]);
        ...
        const [[tableCount]] = await db.query('SELECT COUNT(*) AS count FROM `tables` WHERE restaurant_id = 1');
        ...
        const [[menuCount]] = await db.query('SELECT COUNT(*) AS count FROM menu_items WHERE restaurant_id = 1');
