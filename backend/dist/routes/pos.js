...
function requireTenant(request) {
    ...
    if (!restaurantId)
        throw Object.assign(new Error('Tenant context is required'), { statusCode: 403, code: 'TENANT_REQUIRED' });
    return restaurantId;
}
...
async function ensureTable(connection, restaurantId, tableNo) {
    const [[existing]] = await connection.query('SELECT id FROM `tables` WHERE restaurant_id = ? AND table_number = ? LIMIT 1', [
        restaurantId,
        tableNo,
    ]);
    if (existing)
        return Number(existing.id);
    const [result] = await connection.query('INSERT INTO `tables` (restaurant_id, table_number, status) VALUES (?, ?, ?)', [
        restaurantId,
        tableNo,
        'available',
    ]);
...
async function readOrder(connection, restaurantId, orderId) {
    const [[order]] = await connection.query(`SELECT o.id, o.total_amount, o.status, o.created_at, t.table_number
     FROM orders o JOIN \`tables\` t ON t.id = o.table_id
     WHERE o.id = ? AND o.restaurant_id = ? LIMIT 1`, [orderId, restaurantId]);
    if (!order)
        return null;
    const [items] = await connection.query(`SELECT mi.id, mi.name, mi.category, mi.available, oi.quantity, oi.price
...
posRouter.post('/orders', requireRole('admin', 'waiter', 'cashier'), asyncHandler(async (request, response) => {
    ...
    try {
        ...
        const [menuItems] = await connection.query(`SELECT id, price FROM menu_items WHERE restaurant_id = ? AND available = TRUE AND id IN (${ids.map(() => '?').join(',')}) FOR UPDATE`, [restaurantId, ...ids]);
        ...
        const [result] = await connection.query('INSERT INTO orders (restaurant_id, table_id, waiter_id, total_amount, status) VALUES (?, ?, ?, ?, ?)', [restaurantId, tableId, request.user?.id || null, total, 'pending']);
...
posRouter.patch('/orders/:id', requireRole('admin', 'waiter', 'cashier'), asyncHandler(async (request, response) => {
    const restaurantId = requireTenant(request);
    ...
    try {
        await connection.beginTransaction();
        const [result] = await connection.query('UPDATE orders SET status = ? WHERE id = ? AND restaurant_id = ?', [
...
posRouter.delete('/orders/:id', requireRole('admin', 'waiter', 'cashier'), asyncHandler(async (request, response) => {
    const restaurantId = requireTenant(request);
    const db = await getPool();
    const [result] = await db.query('DELETE FROM orders WHERE id = ? AND restaurant_id = ?', [request.params.id, restaurantId]);
    if (!result.affectedRows) {
        response.status(404).json({ code: 'ORDER_NOT_FOUND', message: 'Order not found' });
        return;
    }
...
posRouter.post('/payments', requireRole('admin', 'cashier'), asyncHandler(async (request, response) => {
    const restaurantId = requireTenant(request);
    ...
    try {
        await connection.beginTransaction();
        const [[table]] = await connection.query('SELECT id FROM `tables` WHERE restaurant_id = ? AND table_number = ? LIMIT 1 FOR UPDATE', [
            restaurantId,
            payload.tableNo,
        ]);
        if (!table) {
        ...
        const [orders] = await connection.query("SELECT id, total_amount FROM orders WHERE restaurant_id = ? AND table_id = ? AND status <> 'completed' FOR UPDATE", [restaurantId, table.id]);
        if (!orders.length) {
            await connection.rollback();
...
posRouter.post('/menu', requireRole('admin', 'cashier'), asyncHandler(async (request, response) => {
    const restaurantId = requireTenant(request);
    const payload = createMenuSchema.parse(request.body);
    const db = await getPool();
    const [result] = await db.query('INSERT INTO menu_items (restaurant_id, name, category, price, available) VALUES (?, ?, ?, ?, ?)', [
        restaurantId,
        payload.name,
        payload.category,
        payload.price,
...
posRouter.patch('/menu/:id', requireRole('admin', 'cashier'), asyncHandler(async (request, response) => {
    const restaurantId = requireTenant(request);
    ...
    const [[item]] = await db.query('SELECT id, name, category, price, available FROM menu_items WHERE id = ? AND restaurant_id = ? LIMIT 1', [
        request.params.id,
        restaurantId,
    ]);
    if (!item) {
...
posRouter.delete('/menu/:id', requireRole('admin', 'cashier'), asyncHandler(async (request, response) => {
    const restaurantId = requireTenant(request);
    const db = await getPool();
    const [result] = await db.query('DELETE FROM menu_items WHERE id = ? AND restaurant_id = ?', [request.params.id, restaurantId]);
    if (!result.affectedRows) {
        response.status(404).json({ code: 'MENU_ITEM_NOT_FOUND', message: 'Menu item not found' });
        return;
    }
