...
function dbStatusToUiStatus(status: string) {
...
function uiStatusToDbStatus(status: string) {
  return status === 'Completed' ? 'completed' : 'pending';
}

async function ensureTable(db: Pool, restaurantId: number, tableNumber: number) {
  const [[existing]] = await db.query<any[]>('SELECT id FROM `tables` WHERE restaurant_id = ? AND table_number = ? LIMIT 1', [
    restaurantId,
    tableNumber,
  ]);
  if (existing) return Number(existing.id);
  const [result] = await db.query<any>('INSERT INTO `tables` (restaurant_id, table_number, status) VALUES (?, ?, ?)', [
    restaurantId,
    tableNumber,
    'available',
  ]);
...
export async function readState(db: Pool, restaurantId = 1) {
  const [menuRows] = await db.query<any[]>('SELECT id, name, category, price, available FROM menu_items WHERE restaurant_id = ? ORDER BY id', [
    restaurantId,
  ]);
  const [userRows] = await db.query<any[]>('SELECT id, name, email, role, is_active FROM users WHERE restaurant_id = ? ORDER BY id', [restaurantId]);
  const [orderRows] = await db.query<any[]>(
    `SELECT o.id, o.table_id, t.table_number, o.total_amount, o.status, o.created_at
     FROM orders o LEFT JOIN \`tables\` t ON t.id = o.table_id WHERE o.restaurant_id = ? ORDER BY o.created_at DESC, o.id DESC`,
    [restaurantId],
  );
  ...
  const [paymentRows] = await db.query<any[]>('SELECT order_id, amount, payment_date FROM payments WHERE restaurant_id = ?', [restaurantId]);
...
export async function replaceState(db: Pool, state: any, restaurantId = 1) {
  ...
  try {
    ...
    if (state.menuItems?.length) {
      await connection.query('INSERT INTO menu_items (id, restaurant_id, name, category, price, available) VALUES ?', [
        state.menuItems.map((item: any) => [item.id, restaurantId, item.name, item.category, item.price, item.available]),
      ]);
    }

    const activeTables = new Set<number>();
    const payloads = [
      ...(state.orders || []).map((order: any) => ({ order, completed: false, paidAt: null, amount: null })),
      ...(state.orderHistory || []).flatMap((entry: any) =>
        entry.orders.map((order: any) => ({ order, completed: true, paidAt: entry.timestamp, amount: entry.grandTotal })),
      ),
    ];

    for (const payload of payloads) {
      const tableId = await ensureTable(db, restaurantId, payload.order.tableNo);
      if (!payload.completed) activeTables.add(tableId);
      ...
      if (payload.order.items?.length) {
        await connection.query('INSERT INTO order_items (order_id, menu_item_id, quantity, price) VALUES ?', [
          payload.order.items.map((item: any) => [payload.order.id, item.menuItem.id || null, item.quantity, item.menuItem.price]),
        ]);
      }
