...
function tenantWhere(request: AuthRequest, configItem: ResourceConfig, values: unknown[]) {
  if (!configItem.tenant || request.user?.role === 'admin') return '';
...
crudRouter.get(
  ...
  asyncHandler(async (request: AuthRequest, response) => {
    ...
    const [rows] = await db.query(
      `SELECT id, restaurant_id, name, email, role, is_active, created_at, updated_at FROM users ${where} ORDER BY id ${query.direction} LIMIT ? OFFSET ?`,
...
crudRouter.post(
  ...
  asyncHandler(async (request: AuthRequest, response) => {
    ...
    const [result] = await db.query<any>(
      'INSERT INTO users (restaurant_id, name, email, password_hash, role, is_active) VALUES (?, ?, ?, ?, ?, ?)',
      [data.restaurant_id ?? request.user?.restaurantId ?? 1, data.name, data.email, hash, data.role, data.is_active ?? true],
    );
...
crudRouter.put(
  ...
  asyncHandler(async (request: AuthRequest, response) => {
    ...
    if (data.restaurant_id !== undefined) updates.restaurant_id = data.restaurant_id;
    if (data.password) updates.password_hash = await bcrypt.hash(data.password, config.auth.bcryptRounds);
...
for (const [name, resource] of Object.entries(resources)) {
  crudRouter.get(
    ...
    asyncHandler(async (request: AuthRequest, response) => {
      ...
      const offset = (query.page - 1) * query.limit;
      const [rows] = await db.query(
        `SELECT * FROM \`${resource.table}\` ${where} ORDER BY id ${query.direction} LIMIT ? OFFSET ?`,
        [...values, query.limit, offset],
      );
      response.json(rows);
    }),
  ...
  crudRouter.get(
    ...
    asyncHandler(async (request: AuthRequest, response) => {
      ...
      let where = 'WHERE id = ?';
      where += tenantWhere(request, resource, values);
      const [rows] = await db.query<any[]>(`SELECT * FROM \`${resource.table}\` ${where} LIMIT 1`, values);
      if (!rows.length) {
  ...
  if (resource.fields.length) {
    crudRouter.post(
      ...
      asyncHandler(async (request: AuthRequest, response) => {
        ...
        if (resource.tenant && request.user?.role !== 'admin') payload.restaurant_id = request.user?.restaurantId;
        const fields = Object.keys(payload);
        const [result] = await db.query<any>(
          `INSERT INTO \`${resource.table}\` (${fields.map((field) => `\`${field}\``).join(', ')}) VALUES (${fields.map(() => '?').join(', ')})`,
