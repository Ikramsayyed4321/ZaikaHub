...
invoicesRouter.post(
  ...
  asyncHandler(async (request: AuthRequest, response) => {
    ...
    const [[order]] = await db.query<any[]>(
      `SELECT o.*, t.table_number FROM orders o LEFT JOIN \`tables\` t ON t.id = o.table_id WHERE o.id = ? LIMIT 1`,
    ...
    await db.query(
      'INSERT INTO invoices (restaurant_id, order_id, invoice_number, tax_amount, discount_amount, grand_total, payment_method) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [order.restaurant_id || request.user?.restaurantId || 1, order.id, invoiceNumber, taxAmount, discountAmount, grandTotal, request.body.payment_method || 'cash'],
    );
...
invoicesRouter.get(
  ...
  asyncHandler(async (request: AuthRequest, response) => {
    ...
    const [[invoice]] = await db.query<any[]>(
      `SELECT i.*, o.table_id, t.table_number FROM invoices i JOIN orders o ON o.id = i.order_id LEFT JOIN \`tables\` t ON t.id = o.table_id WHERE i.order_id = ? ORDER BY i.id DESC LIMIT 1`,
      [request.params.orderId],
    );
    if (!invoice) {
    ...
    const [items] = await db.query<any[]>(
      `SELECT mi.name, oi.quantity, oi.price FROM order_items oi LEFT JOIN menu_items mi ON mi.id = oi.menu_item_id WHERE oi.order_id = ?`,
      [request.params.orderId],
    );
