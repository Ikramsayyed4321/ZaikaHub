...
backupRouter.post(
  ...
  asyncHandler(async (request: AuthRequest, response) => {
    ...
    const [result] = await db.query<any>(
      'INSERT INTO backups (restaurant_id, file_name, file_path, status, created_by) VALUES (?, ?, ?, ?, ?)',
      [request.user?.restaurantId || null, fileName, filePath, 'created', request.user?.id || null],
    );
...
backupRouter.get(
  ...
  asyncHandler(async (request, response) => {
    const db = await getPool();
    const [[backup]] = await db.query<any[]>('SELECT * FROM backups WHERE id = ? LIMIT 1', [request.params.id]);
    if (!backup) {
